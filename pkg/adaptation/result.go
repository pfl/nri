...
func (r *result) adjustNetworkConfigurationChanged(cniconfig []*CNIConfig, plugin string) error {
	//	id := r.request.networkconfiguration.PodSandbox.Id

	if len(cniconfig) == 0 {
		return nil
	}

	//	if err := r.owners.claimNetworkConfigurationChanged(id, plugin); err != nil {
	//		return err
	//	}

	r.reply.cniconfig = cniconfig

	return nil
}

func (r *result) adjustPreSetupNetwork(capabilities []*CNICapabilities, plugin string) error {
	id := r.request.presetupnetwork.Pod.Id
...
func (r *result) adjustPostSetupNetwork(result []*Result, plugin string) error {
	id := r.request.postsetupnetwork.Pod.Id

	if len(result) == 0 {
		return nil
	}

	if err := r.owners.claimPostSetupNetwork(id, plugin); err != nil {
		return err
	}
...
func (ro resultOwners) claimNetworkConfigurationChanged(id, plugin string) error {
	return ro.ownersFor(id).claimNetworkConfigurationChanged(plugin)
}
...
func (ro resultOwners) claimPostSetupNetwork(id, plugin string) error {
	return ro.ownersFor(id).claimPostSetupNetwork(plugin)
}
...
func (o *owners) claimNetworkConfigurationChanged(plugin string) error {
	if other := o.NetworkConfigurationChanged; other != "" {
		return conflict(plugin, other, "adjust network configuration changed")
	}
	o.NetworkConfigurationChanged = plugin
	return nil
}

func (o *owners) claimPreSetupNetwork(plugin string) error {
	if other := o.PreSetupNetwork; other != "" {
		return conflict(plugin, other, "adjust pod sandbox pre setup network")
	}
	o.PreSetupNetwork = plugin
	return nil
}

func (o *owners) claimPostSetupNetwork(plugin string) error {
	if other := o.PostSetupNetwork; other != "" {
		return conflict(plugin, other, "adjust pod sandbox post setup network")
	}
	o.PostSetupNetwork = plugin
	return nil
}
